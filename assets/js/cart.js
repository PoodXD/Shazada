...
function addToCart(proname, id, price) {
    var item = {
        proname: proname,
        id: id,
        price: price,
...
function updateCart() {
    ...
    for (var i = 0; i < cartItems.length; i++) {
        ...
        var pronameCell = document.createElement('td');
        pronameCell.textContent = item.proname;
        row.appendChild(pronameCell);
...
function openCheckoutModal() {
    ...
    for (var i = 0; i < cartItems.length; i++) {
        var item = cartItems[i];
        checkoutURL += "name" + i + "=" + encodeURIComponent(item.proname) + "&";
